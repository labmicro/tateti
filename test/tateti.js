...
describe("Juego de TaTeTi", () => {
  ...
  describe("El primer jugador gana la partida", () => {
    ...
    it("El juego termina con una fila con tres valores iguales", (done) => {
      chai.request(server).put("/empezar").send(juego).end();
      chai.request(server).put("/movimiento").send(movimientoGanaFila[0]).end();
      chai.request(server).put("/movimiento").send(movimientoGanaFila[1]).end();
      chai.request(server).put("/movimiento").send(movimientoGanaFila[2]).end();
      chai.request(server).put("/movimiento").send(movimientoGanaFila[3]).end();
      chai
        .request(server)
        .put("/movimiento")
        .send(movimientoGanaFila[4])
        .end((err, res) => {
          res.should.have.status(200);
          res.should.to.be.json;
          res.body.should.be.a("object");
